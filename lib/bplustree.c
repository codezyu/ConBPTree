...
static val_t bplus_tree_search(struct bplus_tree *tree, key_t key)
{
        ...
        pthread_rwlock_t parentlock=NULL;
        while (node != NULL) {
                if (is_leaf(node)) {
...
//之前的
static int leaf_insert(struct bplus_tree *tree, struct bplus_leaf *leaf, key_t key, val_t data,pthread_rwlock_t* before,int num)
{
        //插入此处不用加写锁
        /* search key location */
        ...
        if (leaf->entries == tree->entries) {
            ...
            pthread_rwlock_wrlock(leaf->lock);
                ...
                /* build new parent */
                if (insert < split) {
                        int ret=parent_node_build(tree, (struct bplus_node *)sibling,
                    ...
                    return ret;
                        ...
                        int ret=parent_node_build(tree, (struct bplus_node *)leaf,
...
{
        struct bplus_node *node = tree->root;
        pthread_rwlock_t before[tree->level];
        int num=0;
        struct bplus_non_leaf *parent=NULL;
        while (node != NULL) {
                if (is_leaf(node)) {
                        struct bplus_leaf *ln = (struct bplus_leaf *)node;
                        return leaf_insert(tree, ln, key, data,before,num);
                } else {
                        struct bplus_non_leaf *nln = (struct bplus_non_leaf *)node;
                        //获取当前节点写锁
                        ...
                        if(num!=0){
                            //如果父节点安全
                            if(parent->children+1<tree->order){
                                for(int i=0;i<num;i++){
                                    //解锁前面所有的节点
                                    pthread_rwlock_unlock(&before[i]);
                                    num=0;
                                }
                            }
                        }
                        ...
                        parent=nln;
                        //加入到祖先节点
                        before[num]=parent->lock;
                        num++;
                }
        }

        //这里有个小bug 没法解决树的第一个节点的读写混乱问题
        /* new root */
...
int bplus_tree_delete(struct bplus_tree *tree, key_t key)
{
        ...
        while (node != NULL) {
                if (is_leaf(node)) {
                        struct bplus_leaf *ln = (struct bplus_leaf *)node;
                        return leaf_remove(tree, ln, key,before,num);
                } else {
                        struct bplus_non_leaf *nln = (struct bplus_non_leaf *)node;
                        //获取当前节点写锁
                        pthread_rwlock_wrlock(nln->lock);
                        ...
                        if(num!=0){
                            //如果父节点安全
                            if(parent->children-1>=(tree->order+1)/2){
                                for(int i=0;i<num;i++){
                                    //解锁前面所有的节点
                                    pthread_rwlock_unlock(&before[i]);
                                    num=0;
...
int bplus_tree_update(struct bplus_tree *tree, key_t key, val_t data)
{
    int i;
    struct bplus_node *node = tree->root;
    struct pthread_rwlock_t *parentlock=NULL;
    while (node != NULL) {
        if (is_leaf(node)) {
            struct bplus_leaf *ln = (struct bplus_leaf *)node;
            i = key_binary_search(ln->key, ln->entries, key);
...
static struct kvPair* kvPairInit(key_t key,val_t val){
    struct kvPair *list= calloc(1,sizeof(*list));
    list->key=key;
    list->val=val;
    list->next=NULL;
    return list;
...
struct kvPair* bplus_tree_get_range(struct bplus_tree *tree, key_t key1, key_t key2)
{
    ...
    while (node != NULL) {
            if (is_leaf(node)) {
                    ...
                    while (ln->key[i] <= max) {
                            if(root==NULL) {
                                root = kvPairInit(ln->key[i], ln->data[i]);
                                list=root;
                            }else{
                                list->next= kvPairInit(ln->key[i], ln->data[i]);
                                list=list->next;
                            }
                            if (++i >= ln->entries) {
                                    if (list_is_last(&ln->link, &tree->list[0])) {
